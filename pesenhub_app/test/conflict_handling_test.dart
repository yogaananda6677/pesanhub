...
void main() {
  ...
  group(
    'Acceptance Criteria #1: Stale Mutation Conflict & Server-Wins Protection',
    () {
      test(
        'Server-wins protects terminal state (COMPLETED) against stale local update',
        () {
          ...
          expect(
            classification.defaultStrategy,
            equals(ResolutionStrategy.serverWins),
          );
      ...
      test(
        'QueueController rejects regression of terminal orders and PAID status',
        () {
          ...
          final staleEvent = serverOrder.copyWith(
            orderStatus: 'PREPARING',
            version: 4,
          );
          ...
          final updateSuccess = controller.updateOrderStatus(
            'ord-3',
            'PREPARING',
          );
  ...
  group('Acceptance Criteria #2: Duplicate Ack and Event Prevention', () {
    test(
      'EventDeduplicator filters duplicate event IDs within bounded capacity',
      () {
    ...
    test(
      'QueueController with duplicate eventId drops redundant processing',
      () {
        final controller = QueueController();
    ...
    test(
      'OrderTimelineEvent.deduplicate removes duplicate events and preserves chronological sequence',
      () {
  ...
  group(
    'Acceptance Criteria #3: Safe vs Unsafe Conflict Resolution Strategies',
    () {
      test(
        'Safe conflict allows clientWins, serverWins, and merge strategies',
        () {
          final localOrder = QueueOrder(
            id: 'ord-safe',
          ...
          expect(
            mergeResolved.takeawayNotes,
            contains('Pedas level 5 | [Lokal: Bungkus terpisah ya]'),
          );
      ...
      test(
        'Unsafe conflict prevents clientWins override and defaults to serverWins',
        () {
          final localOrder = QueueOrder(
            id: 'ord-unsafe',
  ...
  group(
    'Acceptance Criteria #4: Conflict Audit Logging without Sensitive PII',
    () {
      ...
      test(
        'logConflict masks customer phone and records resolution without PII',
        () async {
      ...
      test(
        'QueueController.applyConflictResolution records audit entry automatically',
        () async {
          final controller = QueueController();
          ...
          expect(
            resolved.takeawayNotes,
            contains('Note server | [Lokal: Note local]'),
          );
  ...
  group('Acceptance Criteria #5: Responsive UI States on Mobile & Tablet', () {
    testWidgets(
      'ConflictResolutionDialog renders safe conflict on mobile (390 x 844) without overflow',
      (tester) async {
    ...
    testWidgets(
      'ConflictResolutionDialog renders unsafe conflict on tablet (1024 x 768) without overflow',
      (tester) async {
        ...
        expect(tester.takeException(), isNull);
        expect(
          find.text('Pembaruan Server Wajib (Server-Wins)'),
          findsOneWidget,
        );
    ...
    testWidgets(
      'OrderDetailView displays conflict banner and resolution triggers',
      (tester) async {
        tester.view.physicalSize = const Size(390, 844);
        tester.view.devicePixelRatio = 1.0;
        addTearDown(() => tester.view.resetPhysicalSize());
        ...
        await controller.executeAction(
          ...
          transitionFn: (id, status, ver) async =>
              throw Exception('409 VERSION_CONFLICT'),
