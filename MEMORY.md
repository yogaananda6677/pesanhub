...
| [1D — #6](https://github.com/yogaananda6677/pesanhub/issues/6) | MVP Integration & Release   | IN_PROGRESS | 2026-09-05 | —          | Prototype #120, redesign #121, integrasi #48, contract gate #49/PR #128 |
...
- Menjamin pesan outbound transient failure dijadwalkan ulang secara otomatis dengan exponential backoff dan idempotency key tetap, success ack menutup outbox sehingga retry berikutnya menjadi no-op, permanent failure masuk dead-letter dengan alasan tersanitasi tanpa secret atau PII, serta worker background yang tahan terhadap crash dan restart aplikasi tanpa kehilangan atau menduplikasi job.

**Changed**

- Membuat migrasi database `000015_create_waha_outbox_retry_logging.up.sql` dan `.down.sql` untuk memperluas tabel `order_notifications` dengan kolom `next_retry_at`, `max_attempts` (default 5), `error_category`, status constraint (`PENDING`, `PROCESSING`, `SENT`, `FAILED`, `SUPPRESSED`, `DEAD_LETTER`), serta indeks `idx_order_notifications_outbox`.
- Mengimplementasikan `OutboxWorker` pada `pesenhub_be/internal/notification/worker.go` dengan polling loop, channel non-blocking `Notify()`, klaim batch transaksi pendek berbasis `FOR UPDATE SKIP LOCKED` (zero DB connection pool exhaustion), recovery job saat startup dan berkala (`RecoverStaleProcessing`), re-evaluasi guard opt-out dan conversation pause/handoff sebelum dispatch, serta penutupan outbox saat success ACK.
- Menambahkan fungsi pembantu `CalculateBackoff` (rumus exponential backoff $1\text{s} \times 2^{\text{attempt}-1}$, capped 60s), `ClassifyError` (klasifikasi aman `PERMANENT_VALIDATION`, `PERMANENT_AUTH`, `TRANSIENT_TIMEOUT`, `SESSION_NOT_READY`, `TRANSIENT_PROVIDER`, `TRANSIENT_NETWORK`, `MAX_ATTEMPTS_EXCEEDED`, `UNKNOWN`), dan `SanitizeError` (masking nomor telepon `+6281****7890` dan scrubbing token/api-key).
- Memperluas interface `Store` dan implementasi `PGStore` serta `MemoryStore` dengan method `ScheduleRetry`, `MarkDeadLetter`, `ClaimBatchForProcessing`, dan `RecoverStaleProcessing`.
- Mengintegrasikan `OutboxWorker` dengan `notification.Service` dan memulai worker background goroutine pada `pesenhub_be/cmd/api/main.go`.
- Menambahkan test suite komprehensif di `pesenhub_be/internal/notification/worker_test.go` dan integration test di `store_integration_test.go`.
- Memperbarui `pesenhub_be/scripts/test-migrations.sh` untuk menguji migrasi 000015 up/down/up.
- Membuat dokumentasi arsitektur di `docs/WHATSAPP_OUTBOX_RETRY.md`.

**Validation**

- `go test -v -race ./internal/notification/...`: PASS (seluruh 17 unit/race tests lulus).
- `./scripts/test-migrations.sh`: PASS (15 migrasi up/down/up/down/up tervalidasi di Docker Postgres).
- `./scripts/test-orders.sh`: PASS (seluruh integration tests order, waha, hermes, dan notification tervalidasi di Docker Postgres).
- `./run.sh check`: PASS (seluruh format, vet, dan test modules lulus).
- `flutter test`: PASS (125/125 mobile test cases lulus).

**Next**

- Buka Pull Request untuk Issue #43, pantau CI, merge ke `main`, lalu lanjutkan ke Issue #44.
